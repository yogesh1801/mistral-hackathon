...
Usage:
    python3 llm_battle_finetuned.py --matches 3 --turns 20
...
PLAYERS = [
    {"name": "Ministral 3B", "symbol": "A", "model": "yogesh1801/ministral-3b-grpo-ascii-tanks-merged"},
...
def call_llm(client, model: str, messages: list) -> dict:
    """Call Mistral or OpenAI API and parse JSON response."""
...
def run_match(
    ...
    for turn in range(1, max_turns + 1):
        ...
        # Call LLM
        client_to_use = client_a if pid == 0 else client_b
        try:
            action = call_llm(client_to_use, model, conv)
...
def main():
    ...
    # Mistral client for Player B
    api_key = os.environ.get("MISTRAL_API_KEY")
    if not api_key:
        logger.error("MISTRAL_API_KEY environment variable not set!")
        sys.exit(1)
    client_b = Mistral(api_key=api_key)
    
    # Local OpenAI client for Player A (Finetuned)
    client_a = OpenAI(
    ...
    with open(CSV_FILE, "w", newline="") as f:
        ...
        for match_num in range(1, args.matches + 1):
            winner = run_match(client_a, client_b, match_num, args.turns, args.server, writer)
            if winner in wins:
                wins[winner] += 1
            else:
                wins["draw"] += 1
